...
class SSLTrainer(pl.LightningModule, BuildFromArgparse):
    ...
    def selct_coords(self, array, coords):
        import pdb; pdb.set_trace()
        selected = [x[:, c[0], c[1]] for x, c in zip(array, coords)]
        selected = torch.stack(selected)
        # selected.shape = (batch, channels, num_selections)
        return selected.permute(0, 2, 1)

    def forward_patches(self, x, abs_coords):
        # expects input in the shape of
        # x.shape = 
        # (batch, patch_size, channels, patch_width, patch_height)
        b, p, c, pw, ph = x.shape
        z = self.model.forward(x)
        z_patched = self.selct_coords(z, abs_coords)
        return out_0

    def build_models(self):
        self.model = self.model = UNet2d(self.in_channels, self.out_channels, pad_convs=True)
    ...
    def training_step(self, batch, batch_nb):
        ...
        raw, patches, abs_coords, patch_matches, mask = batch

        emb_0 = self.forward_patches(raw, abs_coords)
