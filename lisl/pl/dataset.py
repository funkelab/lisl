...
class AugmentedZarrDataset(Dataset):
    """
    Attributes
    ----------
    ds_file: str
    ...
    def augment_batch(self, raw, gtseg):
        ...
        if self.augment:
            raw, gtseg = self.batch_augmentation_fn(image=raw, segmentation_maps=gtseg)
        if self.crop_fn is not None:
            raw, gtseg = self.crop_fn(image=raw, segmentation_maps=gtseg)
